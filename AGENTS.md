...
- Current roadmap phase: **roadmap complete (phases 0–5 done)** — see
...
- `src/agent/`: the harness core (Phase 2).
  ...
  - `tools/`: one file per tool, each exporting a `ToolSpec` (`name` + `definition` + `create(ctx)`).
    - `types.ts`: `ToolContext` (optional `campaign` / `session` / `oneshotsDir` / `ask`) and `ToolSpec`. `create` returns `null` when a tool can't run in the given context — that's how campaign tools drop out of a campaign-less agent, and `ask_user` out of a UI-less one.
